...
contract EventTicketsV2 {
    ...
    */
    ...
    modifier paidEnough(uint _qty) { require( msg.value >= PRICE_TICKET * _qty ); _;}
    modifier isOpened(uint _eventid) { require(events[_eventid].isOpen == true ); _;}
    modifier enoughTicket(uint _eventid, uint _qty) { require( events[_eventid].totalTickets - events[_eventid].sales > _qty ); _;}
    modifier refund(uint _qty) { _; msg.sender.transfer(msg.value - PRICE_TICKET * _qty); }
    ...
    */
    ...
    function buyTickets(uint _id, uint _qty) 
        payable
        public
        paidEnough(_qty)
        isOpened(_id)
        enoughTicket(_id,_qty)
        refund(_qty)
    {

        events[_id].buyers[msg.sender] += _qty;
        events[_id].sales += _qty;
        emit LogBuyTickets( msg.sender, _id , _qty );
    ...
    */
    ...
    {
        uint ticketqty = events[_id].buyers[msg.sender];
        events[_id].sales -= ticketqty;
        msg.sender.transfer(ticketqty * PRICE_TICKET);
        emit LogGetRefund(msg.sender, _id, ticketqty);
    }
